...
/// Format rule content for writing to a rule file.
///
/// Wraps the raw rule body with the RuleWeaver marker so the reconciler can
/// distinguish managed files from user-created files when scanning the filesystem.
pub fn format_rule_content_standalone(name: &str, content: &str) -> String {
    format!(
        "<!-- Generated by RuleWeaver - Do not edit manually -->\n\n## {}\n\n{}\n",
        name, content
    )
...
pub fn format_rule_content_aggregate(rules: &[(&str, &str)]) -> String {
    let mut output = "<!-- Generated by RuleWeaver - Do not edit manually -->\n\n".to_string();

    for (idx, (name, content)) in rules.iter().enumerate() {
        if idx > 0 {
            output.push('\n');
        }
...
pub fn format_rule_content(name: &str, content: &str) -> String {
    format_rule_content_standalone(name, content)
}
