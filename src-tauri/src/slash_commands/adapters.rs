...
#[derive(Serialize)]
struct AugmentFrontmatter<'a> {
    description: Cow<'a, str>,
    #[serde(rename = "argument-hint", skip_serializing_if = "Option::is_none")]
    argument_hint: Option<Cow<'a, str>>,
}

fn try_serialize_frontmatter<T: Serialize>(frontmatter: &T) -> Option<String> {
...
fn format_with_frontmatter(frontmatter: &StandardFrontmatter<'_>, script: &str) -> String {
    let yaml = match try_serialize_frontmatter(frontmatter) {
        Some(yaml) => yaml,
...
fn format_with_leading_frontmatter<T: Serialize>(frontmatter: &T, script: &str) -> String {
    let yaml = match try_serialize_frontmatter(frontmatter) {
        Some(yaml) => yaml,
        None => return script.to_string(),
    };
...
/// Augment slash command adapter (Custom Commands)
/// Format: Markdown with optional YAML frontmatter
/// Arguments: Hint only, no argument substitution
pub struct AugmentSlashAdapter;

impl SlashCommandAdapter for AugmentSlashAdapter {
    fn name(&self) -> &'static str {
        AdapterType::Augment.as_str()
    }

    fn file_extension(&self) -> &'static str {
        registry_entry(&AdapterType::Augment)
            .slash_command_extension
            .unwrap_or("md")
    }
...
mod tests {
    ...
    #[test]
    fn test_augment_adapter_uses_optional_frontmatter() {
        let mut command = create_test_command();
        command.arguments.push(crate::models::CommandArgument {
        ...
        let adapter = AugmentSlashAdapter;
        assert_eq!(adapter.name(), "augment");
        assert!(!adapter.supports_argument_substitution());
        ...
        assert!(content.contains("branch-name"));
        assert!(content.contains("npm test"));
    }
